...
# If the user submits a query, run the chain to get a response...
# and update the chat history with the response...
# If query is not empty, append it to the history and run the chain...
# If query is empty, do nothing...
# This is the main part of the app where the user interacts with the LLM...
if query:
    st.session_state.history.append({"role": "user", "text": query})
    try:
        # We have Show a spinner while waiting for the LLM response for Better Response...
        with st.spinner('Waiting for answer...'):
            # Run the chain to get the response from the LLM...
            # This will call the run_chain function from the backend.chains module...
            # The run_chain function will handle the LLM response and return the result and latency...
            result, latency = run_chain(query, st.session_state.history)
    # If there is an error during the LLM response, show an error message...
    # and set a default result and latency...
    # This is to handle cases where the LLM is overloaded or unavailable...
    # We also log the error message for debugging purposes...
    except Exception as e:
        st.error(f"Error during LLM response Ashmin : {e}")
        st.success("We can set an API from the Sidebar for the LLM to use.")
    ...
    # Display the assistant's response in the chat message format...
    # This will show the response from the LLM in the chat interface...
    # It will also show the sentiment and escalation information if applicable...
    # The sources will be displayed as captions below the response...
...
# Metrics Expander for showing the metrics of the LLM response...
# This is useful for monitoring the performance of the LLM and the user interactions...
# This will help in understanding the user sentiment and escalation patterns...
# It will also help in improving the LLM responses and the overall user experience... (Mainly for Development)
with st.expander("Metrics"):
    ...
    # Average latency is displayed here...
    ...
    # Display the number of negative sentiments in the last 10 turns...
