def should_escalate(history, N=2):
    ...
    # Our Rule 1: Last N user messages have NEGATIVE sentiment....
    if all(t.get("sentiment") == "NEGATIVE" for t in lastN):
        return True
    # Our Rule 2: Same user question/complaint repeated in last N.....
    texts = [t.get("text", "").strip().lower() for t in lastN]
    if len(set(texts)) == 1 and texts[0] != "":
        return True
    # Our Rule 3: Any user message in last N has sentiment_score <= 0.3....
    for t in lastN:
        score = t.get("sentiment_score")
        if score is not None and score <= 0.3:
            return True
    # Default Value Parse: No escalation needed....
    return False
...
def escalation_reason(history, N=2):
    ...
    """
    # Reasoning for escalation Trigger.....
    user_turns = [t for t in history if t.get("role") == "user"]
    if len(user_turns) < N:
        return ""
    lastN = user_turns[-N:]
    # Reason Messages for the Escalation......
    if all(t.get("sentiment") == "NEGATIVE" for t in lastN):
    ...
    # Default Reasoning : No specific reason found....
    return ""
